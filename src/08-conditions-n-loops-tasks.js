...
function isInsideCircle(circle, point) {
  const x = Math.abs(point.x - circle.center.x) ** 2;
  const y = Math.abs(point.y - circle.center.y) ** 2;
  const distanceFromCenter = Math.sqrt(x + y);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return (b > a) ? (`${startBracket}${a}, ${b}${endBracket}`) : (`${startBracket}${b}, ${a}${endBracket}`);
}
...
function getMatrixProduct(m1, m2) {
  ...
  for (let index = 0; index < m1.length; index += 1) {
    const row = m1[index];
    const newRow = [];
